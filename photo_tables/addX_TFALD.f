...
!!!!!!!!!!!!!!!!!!initialization call to user subroutine!!!!!!!!!!!!!!
      ...
      call X_TFALD (W(1),XT,XP,XM, XNEW,
...
! primary high-resolution wavelength loop - generate input for pratmo reference J's
      ...
      call X_TFALD (W(J),XT,XP,XM, XNEW,
     &      INIT, MM,TTT,PPP,ISX,ISP,TITLNEW,TITLTBL)
...
c-----------------------------------------------------------------------
      subroutine X_TFALD (WW,XT,XP,XM,XNEW,
     &     INIT, MM,TTT,PPP,ISX,ISP,TITLNEW,TITLTBL)
c-----------------------------------------------------------------------
...
!---this is all table specific formats
          ...
          read(3,'(a)', iostat=ios) line
               if (ios /= 0) then
                   print *, 'Error reading line. IOSTAT = ', ios
                   stop
               end if
               print *, 'Line read: ', trim(line)
               read(line, '(i4,1x,a)') NW, FORMW
